...
export default function LibraryCard({ local, image, title, date, timeInicio, timeFim, onDelete, isSmallCard }) {
  ...
  const handleCardClick = () => {
    if (local?.nome_ambiente && local?.descricao_ambiente && local?.imagem_ambiente && local?.id_ambiente) {
      window.location.href = `/screens/Agendamento?title=${encodeURIComponent(local.nome_ambiente)}&description=${encodeURIComponent(local.descricao_ambiente)}&img=${encodeURIComponent(local.imagem_ambiente)}&id=${encodeURIComponent(local.id_ambiente)}`;
    }
  };
